...
function add_project(path){
    ...
    $('#add_project_comment').val("");
    $('#add_project_summary').val("");
    ...
    $('#add_project_geophysics_date').val("");
    $('#add_project_licensing_date').val("");
    $('#add_project_drilling_date').val("");
    $('#add_project_discovery_date').val("");
    $('#add_project_fid_date').val("");
    $('#add_project_development_date').val("");
    $('#add_project_operational_date').val("");
    $('#add_project_confidential_link').val("");
    $('#add_project_external_link').val("");
...
//    });
    ...
    $('#add_project_country').change(function(){
        $('#add_project_base').html("");
        $.ajax({
            url:        Routing.generate('_search_bases'),
            type:       'POST',
            data:       $('#add_project_country'),
            dataType:   'json',
            success:    function(data){
                var select = $('#add_project_base');
                ...
                if(data['empty'] != 1){
                    $.each(data, function(projectKey, project){
                        projectTable.row.add({
                            ...
                            4 : project.base.name,
                            5 : project.status,
                            6 : project.projectTiming,
                            7 : project.summary,
                            8 : '<button class="btn btn-default btn-sm" onClick="openDetailWindow(Routing.generate(\'_project_detail\', {id:'+project.id+'}))" role="button"><i class="fa fa-folder"></i></button>'
                        }).draw();
