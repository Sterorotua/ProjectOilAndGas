...
function drawProject(base, area, map){
    ...
    if (base.gps.lat < 1000 && base.gps.lng < 1000){
    ...
    if ('operatorGroup' in pointOfInterest){
        var infoWindow = new google.maps.InfoWindow({
            content: '<h3>' + pointOfInterest.name + '</h3>' +
                    ...
                    '<br>Airport Link : <a href="#" onClick=\"window.open(\'' + pointOfInterest.airportLink + '\')">'+ pointOfInterest.airportLink +'</a>'
        });
...
$('.has-error').on('change', function(){
    if($(this).val() !== null || $(this).val() !== ""){
        $(this).attr('class', 'form-group has-success');
    }
    else{
...
$('.has-success').on('change', function(){
    if($(this).val() !== null || $(this).val() !== ""){
        $(this).attr('class', 'form-group has-error');
    }
});
