                ...
                if(data['empty'] != 1){
                    $.each(data, function(tenderKey, tender){
                        tenderTable.row.add({
                            ...
                            3 : tender.oilCompany,
                            4 : tender.status,
                            5 : tender.tenderTiming,
                            6 : tender.summary,
                            7 : '<button class="btn btn-default btn-sm" onClick="openDetailWindow(Routing.generate(\'_tender_detail\', {id:'+tender.id+'}))" role="button"><i class="fa fa-folder"></i></button>'
                        }).draw();
